          ...
          setInterval(() => {
            try {
              ...
              const nearbyTargets = entities.filter(entity => {
                ...
                if (attackPlayers && entity.type === 'player' && entity.username !== MAIN_PLAYER && entity.username !== bot.username && entity.username !== "L9V7" && entity.username !== "NewFrontDoor") {
...
bot.on('physicsTick', () => {
  ...
  // Adjust target speed dynamically
  const maxSpeed = 0.5; // Maximum movement speed
  const minSpeed = 0.1; // Minimum movement speed to avoid stalling
  const targetSpeed = Math.max(minSpeed, Math.min(maxSpeed, distance * 0.5)); // Scales speed based on distance

  const factor = 0.2; // Adjust smoothing factor for faster response

  if (distance > 0.1) {
    bot.entity.velocity.x = lerp(bot.entity.velocity.x, (dx / distance) * targetSpeed, factor);
    bot.entity.velocity.z = lerp(bot.entity.velocity.z, (dz / distance) * targetSpeed, factor);
  } else {
    // Gradually stop the bot when close to the target
    bot.entity.velocity.x = lerp(bot.entity.velocity.x, 0, factor);
    bot.entity.velocity.z = lerp(bot.entity.velocity.z, 0, factor);
  }
});
