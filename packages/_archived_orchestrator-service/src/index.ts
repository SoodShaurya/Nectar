...
let conversationHistory: Array<{ role: string; parts: any[] }> = [];
...
logger.info('Gemini AI planner initialized', { model: PLANNER_MODEL });
...
const orchestratorTools = [
    {
        ...
        parameters: {
            type: Type.OBJECT,
            properties: {
                missionDescription: { type: Type.STRING, description: "High-level description of the mission objective." },
                taskDetails: { type: Type.OBJECT, description: "Specific details or parameters for the mission (e.g., target coordinates, resource type/quantity).", properties: {} },
                requiredAgents: { type: Type.NUMBER, description: "The number of agents required for this mission." },
                selectionCriteria: {
                    type: Type.OBJECT,
                    ...
                    properties: {
                        criteriaType: { type: Type.STRING, description: "Type of criteria ('proximity', 'random', 'tags', 'inventory', etc.)" },
                        targetCoords: {
                            type: Type.OBJECT, description: "Target coordinates for proximity check.",
                            properties: { x: { type: Type.NUMBER }, y: { type: Type.NUMBER }, z: { type: Type.NUMBER } }, required: ['x', 'y', 'z']
                        },
                        requiredTags: { type: Type.ARRAY, description: "List of tags agents must possess.", items: { type: Type.STRING } },
                        requiredInventory: { type: Type.OBJECT, description: "Minimum required items in agent inventory.", properties: {} }
                    }
                },
                suggestedRoles: {
                    type: Type.ARRAY,
    ...
    },
    {
        ...
        parameters: {
            type: Type.OBJECT,
            properties: {
                query: { type: Type.OBJECT, description: "A MongoDB-style query object or parameters for a predefined query.", properties: {} }
            },
            required: ['query']
        }
    },
                ...
                plan: {
                    ...
                    items: {
                        ...
                        properties: {
                            ...
                            status: { type: Type.STRING, enum: ['pending', 'in_progress', 'complete', 'failed'] },
                            objectives: {
                                type: Type.ARRAY,
                                items: {
                                    type: Type.OBJECT,
                                    properties: {
                                        ...
                                        assignedSquad: { type: Type.STRING }
                                    },
...
// --- System Instruction ---
const SYSTEM_INSTRUCTION = `You are the Orchestrator AI for Aetherius, a Minecraft bot swarm. Your goal is to achieve the high-level objective: "Beat the Ender Dragon".
...
async function executeFunctionCall(name: string, args: any): Promise<any> {
    let result: any = { success: false, error: "Unknown error during execution" };
    ...
    try {
        switch (name) {
            case "delegateTaskToSquad": {
                ...
                if (typedArgs.missionDescription && typedArgs.taskDetails && typedArgs.requiredAgents && typedArgs.selectionCriteria) {
                    ...
                    result = { success: true, status: "Squad spawning initiated" };
            ...
            case "requestWorldStateQuery": {
                ...
                if (typedArgs.query) {
                    ...
                    result = { success: true, result: queryResult ?? 'Query failed or returned null' };
        ...
        result = { success: false, error: toolError instanceof Error ? toolError.message : String(toolError) };
    }
...
async function runStrategicPlanning(triggeringEvent?: any) {
    ...
    try {
        ...
        logger.debug('Preparing to call Gemini for strategic planning');
        ...
        if (cachedResponse) {
            ...
            // For cached responses, skip the LLM loop — just re-execute function calls
            ...
            if (cachedFunctionCalls && cachedFunctionCalls.length > 0) {
                for (const call of cachedFunctionCalls) {
                    await executeFunctionCall(call.name!, call.args);
                }
            }
        } else {
            logger.info('Calling Gemini API for strategic planning');
            ...
            while (continueLoop) {
                ...
                const llmResponse = await metrics.measureAsync('llm_strategic_call', async () => {
                    return await geminiCircuitBreaker.execute(async () => {
                        return await genAI.models.generateContent({
                            ...
                            config: {
                                ...
                                tools: [{ functionDeclarations: orchestratorTools as any }],
                ...
                if (functionCalls && functionCalls.length > 0) {
                    ...
                    // Add model response to history (preserve thought signatures)
                    if (llmResponse.candidates?.[0]?.content) {
                        conversationHistory.push(llmResponse.candidates[0].content as any);
                    }

                    // Execute each function call and build responses
                    const functionResponseParts: any[] = [];
                    for (const call of functionCalls) {
                        logger.debug('Executing LLM tool', { tool: call.name!, args: call.args });
                    ...
                    // Cache the first response for this prompt
            ...
            // --- Handle Squad Leader Reports ---
                ...
                if (squad) {
                 ...
                 if (currentFailures >= SQUAD_FAILURE_THRESHOLD) {
                     ...
                     squadFailureCounts.delete(squadId); // Reset count after triggering re-plan
                 }
