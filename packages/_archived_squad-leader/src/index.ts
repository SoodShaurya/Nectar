...
let tacticalConversationHistory: Array<{ role: string; parts: any[] }> = [];
...
const genAI = new GoogleGenAI({ apiKey: GEMINI_API_KEY || "DUMMY_KEY" });
...
const tacticalTools = [
            ...
            properties: {
                commands: {
                    ...
                    items: {
                        type: Type.OBJECT,
                        properties: {
                            agentId: { type: Type.STRING, description: "The unique ID of the agent to command." },
                            taskId: { type: Type.STRING, description: "A unique identifier for this specific task instance." },
                            task: {
                                type: Type.OBJECT,
                                description: "The TaskObject defining the action.",
                                properties: {
                                    type: { type: Type.STRING, description: "The type of task (e.g., NavigateTo, Gather, Attack)." },
                                    details: {
                                        type: Type.OBJECT,
                                        description: "Parameters specific to the task type (e.g., {targetCoords: {x,y,z}} for NavigateTo).",
                                        properties: {}
                                    }
                                },
    ...
    },
    {
        ...
        parameters: {
            type: Type.OBJECT,
            properties: {
                status: { type: Type.STRING, description: "A brief description of the current squad status." },
                progress: { type: Type.NUMBER, description: "Estimated mission progress (0.0 to 1.0)." },
                details: { type: Type.OBJECT, description: "Optional object with additional details.", properties: {} }
            },
    ...
    },
    {
        ...
        description: "Reports a strategically significant finding (like an End Portal Frame, Spawner, Village) to the Orchestrator.",
        parameters: {
            type: Type.OBJECT,
            properties: {
                findingDetails: {
                    ...
                    properties: {
                        poiType: { type: Type.STRING, description: "Type of Point of Interest found." },
                        location: {
                            type: Type.OBJECT,
    ...
    },
    {
        ...
        parameters: {
            type: Type.OBJECT,
            properties: {
                results: { type: Type.OBJECT, description: "An object summarizing the mission results.", properties: {} }
            },
            required: ['results']
        }
    },
    {
        ...
        parameters: {
            type: Type.OBJECT,
            properties: {
                reason: { type: Type.STRING, description: "A brief explanation of why the mission failed." }
            },
            required: ['reason']
        }
    }
...
// --- Mission Timeout ---
const MISSION_TIMEOUT_MS = 10 * 60 * 1000; // 10 minutes
let missionTimeoutHandle: ReturnType<typeof setTimeout> | null = null;

function startMissionTimeout() {
    if (missionTimeoutHandle) clearTimeout(missionTimeoutHandle);
    missionTimeoutHandle = setTimeout(() => {
        logger.error(`Mission timed out after ${MISSION_TIMEOUT_MS / 1000}s`);
        metrics.increment('mission_timeouts');
        declareMissionFailed(`Mission timed out after ${MISSION_TIMEOUT_MS / 1000} seconds`);
    }, MISSION_TIMEOUT_MS);
}

function clearMissionTimeout() {
    if (missionTimeoutHandle) {
        clearTimeout(missionTimeoutHandle);
        missionTimeoutHandle = null;
    }
}

// --- Tactical Planning System Instruction ---
function buildSystemInstruction(): string {
    return `You are a Squad Leader AI responsible for the tactical execution of a mission using a team of Minecraft bot agents.
...
// --- Execute tactical function call ---
function executeTacticalFunctionCall(name: string, args: any): void {
    switch (name) {
...
async function runTacticalPlanning(triggeringEvent?: any) {
    ...
    try {
        // Build User Prompt for this turn
        ...
        if (cachedResponse) {
            ...
            // Re-execute cached function calls
            const cachedFunctionCalls = cachedResponse.functionCalls;
            if (cachedFunctionCalls && cachedFunctionCalls.length > 0) {
                for (const call of cachedFunctionCalls) {
                    executeTacticalFunctionCall(call.name!, call.args);
                }
            }
        } else {
            ...
            tacticalConversationHistory.push({ role: 'user', parts: [{ text: userPrompt }] });

            // Rate limit
            await geminiRateLimiter.waitIfNeeded();

            // Circuit breaker + metrics
            const llmResponse = await metrics.measureAsync('llm_tactical_call', async () => {
                return await geminiCircuitBreaker.execute(async () => {
                    return await genAI.models.generateContent({
                        model: TACTICAL_MODEL,
                        contents: tacticalConversationHistory as any,
                        config: {
                            systemInstruction: buildSystemInstruction(),
                            tools: [{ functionDeclarations: tacticalTools as any }],
            ...
            if (llmResponse.candidates?.[0]?.content) {
                tacticalConversationHistory.push(llmResponse.candidates[0].content as any);
            }
            ...
            if (functionCalls && functionCalls.length > 0) {
                ...
                logger.info("LLM Response: No function calls. Text:", responseText);
                if (responseText) {
                    reportStatusToOrchestrator(`LLM Update: ${responseText}`, -1);
                }
            }
        }
    } catch (error) {
        ...
        reportStatusToOrchestrator(`LLM tactical planning failed: ${errorMessage}`, -1, { error: true });
    } finally {
        metrics.record('tactical_planning_duration', Date.now() - planningStartTime);
        isPlanningRunActive = false;
    }
}
